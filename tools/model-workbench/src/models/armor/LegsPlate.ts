...
export class LegsPlate implements Model {
  ...
  private drawLeg(g: Graphics, j: Record<string, V>, p: any, s: number, side: "L" | "R"): void {
    ...
    // Cuisse (thigh plate)
    drawTaperedLimb(g, legTop, knee, 6.8, 5.2, p.body, p.bodyDk, p.outline, s);

    // Plate edge highlight
    drawTaperedLimb(g, legTop, knee, 6.8, 5.2, p.bodyLt, p.bodyLt, p.bodyLt, s);
    // Redraw main over highlight to create edge effect
    drawTaperedLimb(g, legTop, { x: knee.x, y: knee.y - 0.5 }, 6, 4.5, p.body, p.bodyDk, p.outline, s);
    ...
    // Greave (shin plate)
    drawTaperedLimb(g, knee, ankle, 5.5, 4.2, p.body, p.bodyDk, p.outline, s);
