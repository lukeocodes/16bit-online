...
export class WallEwin implements Model {
  readonly id         = "wall-e-win";
  readonly name       = "Wall E Win";
  ...
  getDrawCalls(ctx: RenderContext): DrawCall[] {
    ...
    quadWin(DEPTH_E + 1, LIT_COL, [IA, IB, lift(IB), lift(IA)], innerHole);
    if (iso.x <= 0) { quad(DEPTH_E + 2, SIDE_COL, [OA, IA, lift(IA), lift(OA)]); }
    if (iso.y >= 0) { quadWin(DEPTH_E + 3, DIM_COL, [OA, OB, lift(OB), lift(OA)], outerHole); }
    quad(DEPTH_E + 4, TOP_COL, [lift(OA), lift(OB), lift(IB), lift(IA)]);
    if (iso.x >= 0) { quad(DEPTH_E + 5, SIDE_COL, [OB, IB, lift(IB), lift(OB)]); }

    // Window reveals — drawn before the inner face so the inner face renders over them in solid areas
    quad(DEPTH_E + 0, HEAD_COL, [
      outerUV(WIN_U0, WIN_V1), outerUV(WIN_U1, WIN_V1),
      innerUV(WIN_U1, WIN_V1), innerUV(WIN_U0, WIN_V1),
    ]);
    quad(DEPTH_E + 0, JAM_COL, [
